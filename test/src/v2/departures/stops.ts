  ...
  for (let test of testData.scenarios) {
    ...
    let expects: ExpectsType = [
      { check: "should have status 200", expect: res.status === 200 },
      {
        ...
        expect:
          (<JSONArray>res.json(`nearest.edges`)).length ===
          test.expectedResult.stopPlaces.length,
      },
      {
        check: "should have correct stop places",
        expect:
          (<JSONArray>res.json(`nearest.edges.#.node.place.id`)).toString() ===
...
export function stopsDetails(testData: stopsDetailsTestDataType) {
  for (let test of testData.scenarios) {
    ...
    for (let expResult of test.expectedResults) {
      // Names and ids
      let resStopPlaceNames = (<JSONArray>(
        res.json(`stopPlaces.#(id="${expResult.stopPlaceId}")#.name`)
      )).toString();
      let resQuayIds = (<JSONArray>(
        (<JSONArray>(
          res.json(`stopPlaces.#(id="${expResult.stopPlaceId}")#.quays.#.id`)
        ))[0]
      ))
        .sort()
        .toString();
