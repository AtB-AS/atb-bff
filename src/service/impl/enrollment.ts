...
const throttledFetch = throttle((url: RequestInfo, init?: RequestInit | undefined) => {
...
const request = async <T>(method: string, url: string, payload: any): Promise<Result<T, APIError>> => {
  try {
    const response = await throttledFetch(url, {
      method: 'POST',
      headers: typeof payload == undefined ? undefined : {'Content-Type': 'application/json'},
...
const service = (): IEnrollmentService => {
  ...
  return {
    enroll: async (customerAccountId: string, enrollmentId: string, code: string) => {
      if (customerAccountId.length < 1) {
        return Result.err(new APIError({message: 'Invalid customer account id'}));
      }
