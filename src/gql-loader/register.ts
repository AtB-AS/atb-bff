...
const VALID_EXTENSIONS = ['graphql', 'gql'];
...
const handleModule = (m: NodeJS.Module, filename: string) => {
  ...
  for (const op of wDoc.definitions) {
    if (op.kind === 'OperationDefinition') {
      ...
      m.exports.operations[opName] = oneQuery(definitionRefs, wDoc, opName);
    }

    if (op.kind === 'FragmentDefinition') {
      const opName = op.name.value;
      // Print and reparse to nullify loc data
      m.exports.fragments[opName] = gql`
